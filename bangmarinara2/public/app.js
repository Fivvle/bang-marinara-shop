...
const PRODUCTS = [
  ...
  },
  {
    ...
    bg: '#f2efe8',
    color: '#0a0a0a',
    desc: 'Boxy-cut heavyweight cotton shirt. Triple-stitched seams. Slightly oversized through the torso. Washes better with age.',
  ...
  },
  {
    ...
    bg: '#3d3530',
    color: '#f2efe8',
    desc: 'Wide-leg cotton-linen trouser. Elastic waist with optional belt loops. Two deep side pockets. Stone & Ink.',
    sizes: ['XS','S','M','L','XL'],
  },
  {
    id: 4,
    ...
    bg: '#5a6348',
    color: '#f2efe8',
    desc: 'Ripstop shell, four chest pockets, single back vent. Wears loose and unlined. Sage green.',
    sizes: ['S','M','L','XL'],
  },
  {
    id: 5,
    ...
    bg: '#b84a2e',
    color: '#f2efe8',
    desc: 'Relaxed heavyweight jersey. Drop shoulder. Screen-printed archive mark at left chest. Fade-resistant dye.',
    sizes: ['XS','S','M','L','XL','XXL'],
  },
  {
    id: 6,
  ...
  },
  {
    ...
    bg: '#c4b8a8',
    color: '#0a0a0a',
    desc: 'Merino wool ribbed knit, slim but not tight. Crew neck. No logo. Wears under everything.',
    sizes: ['XS','S','M','L','XL'],
  },
  {
    id: 8,
  ...
  },
  {
    ...
    bg: '#7a7469',
    color: '#f2efe8',
    desc: 'Hand-loomed 100% merino. 200cm length. No fringe. Does exactly one job and does it well.',
    sizes: ['ONE SIZE'],
  },
];
  ...
  grid.innerHTML = list.map(p => `
    <div class="product-card" onclick="openProduct(${p.id})">
      <div class="product-img">
        <div class="product-img-inner" style="background:${p.bg};color:${p.color}">
          ${p.emoji}
        </div>
      </div>
...
function openCart() {
  ...
  if (cart.length === 0) {
    items.innerHTML = '<p style="font-family:var(--font-mono);font-size:13px;color:var(--mid);padding:20px 0">Your bag is empty.</p>';
    total.innerHTML = '';
  } else {
...
async function checkout() {
  if (!currentUser) {
    document.getElementById('cart-msg').textContent = 'Please sign in to checkout.';
    return;
  }
  if (cart.length === 0) {
...
function updateAuthUI() {
  ...
  if (currentUser) {
    ...
    userDisplay.textContent = currentUser.name.split(' ')[0].toUpperCase();
  } else {
